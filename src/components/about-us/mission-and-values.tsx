                ...
                <MaxContainer>
                    {
                        missionAndValues.map((item, index) => {
                            ...
                            return (
                                <div key={item.title} className={cn('flex items-center gap-2 py-10 lg:py-16', {
                                    'flex-col lg:flex-row-reverse ': isOdd,
                                    'flex-col-reverse lg:flex-row': !isOdd
                                })}>
                                    <div className='space-y-4 w-full max-w-[642px]'>
                                        <h2 className='text-lg lg:text-2xl text-[#292929]'>{item.title}</h2>
                                        <h3 className='font-semibold text-xl lg:text-4xl'>{item.subTitle}</h3>
                                        <p className='text-[#292929] lg:text-xl w-full max-w-[587px]'>{item.description}</p>
                                    </div>
                                    <div className='relative w-full h-full max-w-[559px] max-h-[507px]'>
